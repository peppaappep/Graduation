...
(function(dsfa) {
	...
	dsfa.Flow = function(options) {
		...
		this.init = function() {
			...
			// 获取flow.json数据
			dsf.http.request_sync(dsf.url.getWebPath(options.configPath,true), null, "GET")
				.done(function(response) {
					self.flowConfig = response;
				})
		...
		this.zoom = function(zoomValue) {
			...
			if (zoom < 1) {
				...
				self.svgRoot.width(self.root.width() / zoom - svgRootOffset.left).height(self.root.height() / zoom -
					svgRootOffset.top);
			} else {
				//移除所有因缩放而附加的样式
		...
		function clickPanelHander(evt) {
			...
			if (evt.shiftKey) {
				...
				globalPos = getMousePointByCanvas({
					x: evt.pageX,
					y: evt.pageY
				});
				globalMoveLastMouse = {
		...
		function lineMouseDownHandler(evt) {
			...
			if (segmen) {
				...
				$(document).bind("mousemove.dragline", function(evt) {
					var moveMp = getMousePointByCanvas({
						x: evt.pageX,
						y: evt.pageY
					});
					//移动为超过2像素视为无效
			...
			var _drawLineEvent = {
				"mousemove": function(evt) {
					...
					if (target.tagName == "DIV") {
						...
						var node = DSFA.Node.getObjectByTarget(evt.target);
						if (node) {
							var info = getDrawLineEndPointInfo(node, {
								x: evt.pageX,
								y: evt.pageY
							});
							var isAlway = false;
				...
				"mouseout": function(evt) {
					startInfo = getDrawLineStartPointInfo(node, {
						x: evt.pageX,
						y: evt.pageY
					});
			...
			$(document).bind("mousemove.nodedragdrop", function(evt) {
				if (ismousedown) {
					...
					if (isMove) {
						...
						showSamePositionNodeBaseLine(node, {
							x: p.x,
							y: p.y
						});
						autoScroll("node", {
				...
				if (!currentDragLine.changeJoinDriection) {
					...
					if (segmen) {
						...
						if (currentDragLine.dragPosition == "start") {
							...
							//如果没有开始节点，创建一个虚拟的对象
							if (f_node) {
								startInfo = getDrawLineStartPointInfo(f_node, {
									x: evt.pageX,
									y: evt.pageY
								});
								startPoint = startInfo.point;
							...
							if (t_node) {
								endInfo = getDrawLineStartPointInfo(t_node, {
									x: evt.pageX,
									y: evt.pageY
								});
		...
		function getNodeLinkNodPoints(info) {
			...
			if (fromDirection == LineDirection.Right && targetDirection == LineDirection.Left) {
				...
				} else if (direction == _Direction.Bottom || direction == _Direction.Top || direction == _Direction.Left ||
					direction == _Direction.LeftTop || direction == _Direction.LeftBottom) {
					points.push({
						x: sp.x,
						y: sp.y
					});
			...
			//起始节点右侧链接结束节点右侧
				...
				if (direction == _Direction.Right || direction == _Direction.Bottom || direction == _Direction.Top || direction ==
					_Direction.RightTop || direction == _Direction.RightBottom) {
					points.push({
						x: sp.x,
						y: sp.y
					});
			...
			//起始节点右侧链接结束节点上侧
				...
				if (direction == _Direction.Right || direction == _Direction.Top || direction == _Direction.RightBottom ||
					...
					if (targetNode.point.y > sp.y) {
						...
						if (targetNode.point.x <= fromNode.point.x + fromNode.width && targetNode.point.x + targetNode.width >=
							fromNode.point.x) {
							//无间隙
				...
				} else if (direction == _Direction.Left || direction == _Direction.Bottom || direction == _Direction.LeftTop ||
					direction == _Direction.LeftBottom) {
					points.push({
						x: sp.x,
						y: sp.y
					});
					if ((direction != _Direction.LeftBottom && direction != _Direction.Bottom) && targetNode.point.x + targetNode.width >
						fromNode.point.x + fromNode.width) {
						points.push({
			...
			//起始节点右侧链接结束节点下侧
				...
				if (direction == _Direction.Right || direction == _Direction.Bottom || direction == _Direction.RightBottom ||
					direction == _Direction.RightTop) {
					points.push({
						x: sp.x,
						y: sp.y
				...
				} else if (direction == _Direction.Left || direction == _Direction.Top || direction == _Direction.LeftTop ||
					direction == _Direction.LeftBottom) {
					points.push({
						x: sp.x,
						y: sp.y
					});
					if ((direction != _Direction.LeftTop && direction != _Direction.Top) && targetNode.point.x + targetNode.width >
						fromNode.point.x + fromNode.width) {
						points.push({
							x: targetNode.point.x + targetNode.width + offset,
							y: sp.y
			...
			//起始节点下侧链接结束节点下侧
				...
				if (direction == _Direction.Right || direction == _Direction.RightTop || direction == _Direction.Top || direction ==
					_Direction.LeftTop) {
					if (targetNode.point.y + targetNode.height > fromNode.point.y + fromNode.height) {
						offset_new = (targetNode.point.y + targetNode.height) - (fromNode.point.y + fromNode.height) + offset;
					}
				...
				} else if (direction == _Direction.Left || direction == _Direction.RightBottom || direction == _Direction.LeftBottom ||
					...
					if (targetNode.point.x - offset <= sp.x && targetNode.point.x + targetNode.width + offset > sp.x) {
						if (targetNode.point.y - (fromNode.point.y + fromNode.height) > offset || targetNode.point.y - (fromNode.point.y +
								fromNode.height) <= 0) {
							nextP.y = sp.y + offset;
							points.push(nextP);
						} else {
			...
			//起始节点下侧链接结束节点右侧
				...
				if (direction == _Direction.Top || direction == _Direction.Bottom || direction == _Direction.Right || direction ==
					_Direction.RightBottom || direction == _Direction.RightTop) {
					if (targetNode.point.y - (fromNode.point.y + fromNode.height) > 0) {
						//有间隙
						p = points[points.length - 1];
						nextP = {
			...
			//起始节点下侧链接结束节点上侧
				...
				if (direction == _Direction.Top || direction == _Direction.Right || direction == _Direction.Left || direction ==
					_Direction.RightTop || direction == _Direction.LeftTop) {
					...
					if (targetNode.point.x <= (fromNode.point.x + fromNode.width) && (targetNode.point.x + targetNode.width) >=
						...
						if (tcenter.x > sp.x) {
							...
							nextP.x = (fromNode.point.x + fromNode.width < targetNode.point.x + targetNode.width) ? targetNode.point.x +
								targetNode.width : fromNode.point.x + fromNode.width;
							nextP.x += offset;
						}
						points.push(nextP);
			...
			//起始节点左侧对结束节点上侧
				...
				if (direction == _Direction.Right || direction == _Direction.RightTop || direction == _Direction.RightBottom ||
					direction == _Direction.Bottom) {
			...
			//起始节点左侧对结束节点右侧
				...
				if (direction == _Direction.Right || direction == _Direction.RightTop || direction == _Direction.RightBottom ||
					direction == _Direction.Top || direction == _Direction.Bottom) {
					points.push({
						x: sp.x - offset,
						y: sp.y
					});
					...
					if (targetNode.point.y <= fromNode.point.y + fromNode.height && targetNode.point.y + targetNode.height >=
						fromNode.point.y) {
						var tcenter = tools.getNodeCenter(targetNode)
						if (tcenter.y > sp.y) {
							nextP.y = fromNode.point.y < targetNode.point.y ? fromNode.point.y - offset : targetNode.point.y - offset;
						} else {
							nextP.y = fromNode.point.y + fromNode.height < targetNode.point.y + targetNode.height ? targetNode.point.y +
								targetNode.height + offset : fromNode.point.y + fromNode.height + offset;
						}
						points.push(nextP);
			...
			//起始节点左侧对结束节点下侧
				...
				if (direction == _Direction.Top || direction == _Direction.Bottom || direction == _Direction.Right || direction ==
					...
					if (targetNode.point.x + targetNode.width < fromNode.point.x) {
						...
						nextP.y = (targetNode.point.y + targetNode.height) > (fromNode.point.y + fromNode.height) ? (targetNode.point.y +
							targetNode.height) + offset : (fromNode.point.y + fromNode.height) + offset;
						points.push(nextP);
			...
			//起始节点上侧对结束节点左侧
				...
				} else if (direction == _Direction.Right || direction == _Direction.RightBottom || direction == _Direction.Bottom ||
					direction == _Direction.LeftBottom) {
					p = points[points.length - 1];
					nextP = {
						x: p.x,
			...
			//起始节点上侧对结束节点右侧
				...
				if (direction == _Direction.Top || direction == _Direction.Right || direction == _Direction.Bottom || direction ==
					_Direction.RightTop || direction == _Direction.RightBottom) {
					//有间隙
					if (targetNode.point.y + targetNode.height < fromNode.point.y) {
						p = points[points.length - 1];
			...
			//起始节点上侧对结束节点上侧
				...
				if (direction == _Direction.Right || direction == _Direction.Left || direction == _Direction.RightBottom ||
					direction == _Direction.Bottom || direction == _Direction.LeftBottom) {
					p = points[points.length - 1];
					nextP = {
						x: p.x,
						y: p.y
			...
			//起始节点上侧对结束节点下`侧
				...
				if (direction == _Direction.Top || direction == _Direction.LeftTop || direction == _Direction.RightTop) {
				...
				} else if (direction == _Direction.Left || direction == _Direction.Right || direction == _Direction.LeftBottom ||
					direction == _Direction.RightBottom || direction == _Direction.Bottom) {
					p = points[points.length - 1];
					nextP = {
						x: p.x,
						y: p.y
					...
					//无间隙
					if (targetNode.point.x <= fromNode.point.x + fromNode.width && targetNode.point.x + targetNode.width >= fromNode
						.point.x) {
						if (targetNode.point.y < fromNode.point.y) {
							nextP.y = targetNode.point.y - offset;
						} else {
						...
						if (tcenter.x < sp.x) {
							...
							nextP.x = targetNode.point.x + targetNode.width < fromNode.point.x + fromNode.width ? fromNode.point.x +
								fromNode.width + offset : targetNode.point.x + targetNode.width + offset;
						}
						points.push(nextP);
		...
		function updateLineLinkInfo(line, fromNode, toNode, fromDirection, toDirection) {
			...
			if (fromNode && toNode) {
				...
				var ep = {
					x: line.path[line.path.length - 1].x,
					y: line.path[line.path.length - 1].y
				};
	...
	DSFA.Lane = function(flow, opt) {
		...
		this.render = function() {
			if (!this.isRender) {
				...
				var plus_left = $("<img class='plus left' src='/dsfa/res/dsf_styles/themes/workFlow/addLane.png'/>").appendTo(
					title);
				var plus_right = $("<img class='plus right' src='/dsfa/res/dsf_styles/themes/workFlow/addLane.png'/>").appendTo(
					title);
			...
			if (next && next.type != this.type) {
				...
				if (line_length <= alwaySzie || n_line_length <= alwaySzie) {
					...
					if (this.type == _LineType.vertical) {
						...
						if ((this.direction == "bottom" && next.direction == "left") || (this.direction == "top" && next.direction ==
								"right")) {
							sweep = 1;
						} else if ((this.direction == "top" && next.direction == "left") || (this.direction == "bottom" && next.direction ==
								"right")) {
							sweep = 0;
						}
						arr.push({
						...
						arr.push(obj1);
						var sweep = null; //圆弧的顺时针和逆时针 1为顺，0为逆
						if ((this.direction == "left" && next.direction == "top") || (this.direction == "right" && next.direction ==
								"bottom")) {
							sweep = 1;
						} else if ((this.direction == "left" && next.direction == "bottom") || (this.direction == "right" && next.direction ==
								"top")) {
							sweep = 0;
						}
						arr.push({
	...
	var tools = {
		...
		"getDirection": function(startPoint, endPoint) {
			...
			var deg = this.getAngle({
				...
				x: parseFloat(endPoint.x),
				y: parseFloat(endPoint.y)
			});
			if (deg == 0 || deg == 360) {
