...
(function($) {
	...
	$.pt = $.pureToolTips = function(options) {
		...
		function leaveClose() {
			//先解绑再绑定，不然会形成事件链
			$pt.unbind('mouseleave.tooltips');
			$pt.bind("mouseleave.tooltips",function(e) {
				$pt.hide();
				$.isFunction(opts.close) && opts.close();
			});
			$pt.unbind('mouseenter.tooltips')
			$pt.bind("mouseenter.tooltips",function() {
				window.ptt && clearTimeout(ptt);
			});
		}
		return init();
	...
	$.fn.pt = $.fn.pureToolTips = function(options) {
		...
		return this.each(function() {
			$(this).bind("mouseenter.tooltips",function() {
				window.ptt && clearTimeout(ptt);
				window.pta && clearTimeout(pta);
				opts.target = this;
				$.pt(opts);
			});
			$(this).bind("mouseleave.tooltips",function() {
				$('.pt').hide();
				$.isFunction(opts.close) && opts.close();
